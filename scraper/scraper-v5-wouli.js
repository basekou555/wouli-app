...
class WouliScraperV5 {
  ...
  async scrapeAccount(account) {
    ...
    try {
      ...
      // FIX v5.2 : Si 0 posts trouvés, Instagram throttle probablement la session
      if (postsToAnalyze.length === 0) {
        console.log('   ⚠️  0 posts — possible throttling Instagram, cookies rafraîchis');
        await this.saveCookies();
      }
      ...
      // FIX v5.2 : Sauvegarder les cookies après chaque compte traité
      await this.saveCookies();
  ...
  async analyzePost(postUrl, account) {
    ...
    try {
      ...
      const postData = await this.page.evaluate(() => {
        ...
        // FIX v5.2 : Priorité à l'URL og:image Instagram (évite le bug base64 800k chars)
        let imageUrl = '';
      ...
      console.log(`      Image: ${postData.imageUrl ? 'Trouvée (URL Instagram)' : 'Non trouvée'}`);

      // FIX v5.2 : Screenshot UNIQUEMENT si aucune image Instagram trouvée
      if (!postData.imageUrl) {
        try {
          console.log('      Pas d\'og:image, tentative screenshot...');
          const article = await this.page.$('article');
          if (article) {
            const screenshot = await article.screenshot({ encoding: 'base64', type: 'jpeg', quality: 80 });
            if (screenshot) {
              postData.imageUrl = `data:image/jpeg;base64,${screenshot}`;
              console.log('      Screenshot fallback capturé');
            }
          }
  ...
  async parseProgram(postData, account) {
    ...
    if (events.length === 0) {
      ...
      return [this.createManualReviewEvent(postData, account, 'Programme non parsable automatiquement (regex + OCR échoués)')];
    }
  ...
  createManualReviewEvent(postData, account, reason) {
    ...
    return {
      ...
      // FIX v5.2 : description tronquée à 500 chars, jamais de base64 dans les logs
