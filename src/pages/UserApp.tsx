  ...
  if (selectedEvent) {
    return <div className="min-h-screen bg-gray-50 pb-20">
        ...
        <div className="bg-white shadow-sm p-4 flex items-center">
          <Button variant="ghost" size="icon" onClick={() => setSelectedEvent(null)} className="mr-3">
        ...
        <div className="p-4">
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="relative h-64">
              <img src={selectedEvent.image_url || "https://picsum.photos/400/200?random=event"} alt={selectedEvent.title} className="w-full h-full object-cover" />
              ...
              <div className="absolute bottom-4 left-4 right-4">
                <div className="bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-medium text-gray-700 inline-flex items-center">
                  ...
                  {selectedEvent.source === 'business' && <span className="ml-2 bg-orange-100 text-orange-800 px-2 py-0.5 rounded-full text-xs">
                      Établissement
                    </span>}
                </div>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {selectedEvent.time && <div className="flex items-center text-gray-600">
              ...
              {selectedEvent.price_text && <div className="flex items-center text-gray-600">
              ...
              {selectedEvent.tags && selectedEvent.tags.length > 0 && <div className="border-t pt-4">
                  <h3 className="font-medium text-gray-900 mb-2">Tags</h3>
                  <div className="flex flex-wrap gap-2">
                    {selectedEvent.tags.map((tag, index) => <span key={index} className="bg-purple-100 text-purple-800 px-2 py-1 rounded-full text-xs">
                        #{tag}
                      </span>)}
                  </div>
                </div>}
              ...
              <div className="border-t pt-4">
                ...
                {selectedEvent.max_participants && <p className="text-xs text-gray-500">Places limitées à {selectedEvent.max_participants} personnes</p>}
              </div>
            </div>
  ...
  return <div className="min-h-screen bg-gray-50 flex flex-col pb-20">
      ...
      <div className="bg-white shadow-sm p-4">
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="icon" onClick={() => setShowFilters(!showFilters)} className="relative">
            ...
            {selectedCategory !== 'all' && <div className="absolute -top-1 -right-1 w-3 h-3 bg-purple-500 rounded-full"></div>}
          </Button>
        </div>
      </div>

      {/* Filters */}
      {showFilters && <div className="bg-white border-b p-4">
          <div className="flex gap-2 overflow-x-auto pb-2">
            {categories.map(category => <Button key={category.id} variant={selectedCategory === category.id ? "default" : "outline"} size="sm" onClick={() => handleCategoryChange(category.id)} className="whitespace-nowrap">
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="w-full max-w-sm">
          {currentEvent ? <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
              {/* Image */}
              <div className="relative h-96">
                <img src={currentEvent.image_url || "https://picsum.photos/400/200?random=event"} alt={currentEvent.title} className="w-full h-full object-cover" />
                ...
                <div className="absolute top-4 left-4">
                  <div className="bg-white/90 backdrop-blur-sm px-2 py-1 rounded-full text-xs font-medium text-gray-700 flex items-center">
                    ...
                    {currentEvent.source === 'business' && <span className="ml-1 text-orange-600">★</span>}
                  </div>
                </div>
                <div className="absolute bottom-4 left-4 right-4 text-white">
                  ...
                  <div className="space-y-1">
                    ...
                    {currentEvent.price_text && <div className="flex items-center text-sm">
              ...
              <div className="flex justify-center space-x-3 p-4">
                <Button variant="outline" size="icon" className="h-14 w-14 rounded-full border-2 border-red-400 text-red-500 hover:bg-red-50" onClick={handlePass}>
                  <X className="h-6 w-6" />
                </Button>
                
                <Button variant="outline" size="icon" className="h-14 w-14 rounded-full border-2 border-blue-400 text-blue-500 hover:bg-blue-50" onClick={handleParticipate}>
                  <Calendar className="h-6 w-6" />
                </Button>
                <Button variant="outline" size="icon" className="h-14 w-14 rounded-full border-2 border-green-400 text-green-500 hover:bg-green-50" onClick={handleLike}>
                  <Heart className="h-6 w-6" />
                </Button>
              </div>
            </div> : <div className="text-center p-8">
              <p className="text-gray-500">Aucun événement disponible dans cette catégorie</p>
              <Button variant="outline" onClick={() => handleCategoryChange('all')} className="mt-4">
