...
const UserApp = () => {
  ...
  const handleDislike = useCallback(
    (eventId: string, currentIndex: number) => {
      toast({
  ...
  const handleLike = useCallback(
    async (eventId: string, currentIndex: number) => {
        ...
        {filteredEvents.length > 0 ? (
          <>
            {filteredEvents.map((event, index) => (
              <div key={event.id} className="h-screen snap-start snap-always" data-index={index}>
                <EventCard
                  event={event}
          ...
          <div className="h-screen flex items-center justify-center">
            <div className="text-center p-8 space-y-4">
              <p className="text-muted-foreground mb-4">
                {hasActiveFilters ? "Aucun événement ne correspond à tes filtres" : "Aucun événement disponible"}
              </p>
              {hasActiveFilters ? (
                <Button onClick={handleResetFilters}>Réinitialiser les filtres</Button>
              ) : (
                <Button onClick={() => handleCategoryChange("all")}>Voir tous les événements</Button>
              )}
            </div>
          </div>
        )}
