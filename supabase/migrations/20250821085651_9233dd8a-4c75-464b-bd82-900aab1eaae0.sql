...
DO $$
BEGIN
    -- Add unique constraint to event_likes if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'event_likes_user_event_unique'
        AND table_name = 'event_likes'
    ) THEN
        ALTER TABLE public.event_likes 
        ADD CONSTRAINT event_likes_user_event_unique 
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'event_participants_user_event_unique'
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'events_pending_title_length'
        AND table_name = 'events_pending'
    ) THEN
        ALTER TABLE public.events_pending
        ADD CONSTRAINT events_pending_title_length 
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'events_pending_description_length'
        AND table_name = 'events_pending'
    ) THEN
        ALTER TABLE public.events_pending
        ADD CONSTRAINT events_pending_description_length 
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'events_pending_location_length'
        AND table_name = 'events_pending'
    ) THEN
        ALTER TABLE public.events_pending
        ADD CONSTRAINT events_pending_location_length 
        CHECK (length(location) BETWEEN 2 AND 100);
    END IF;
END $$;

-- 8. Add validation trigger for external URLs
...
-- 9. Add similar validation to business_events
DO $$
BEGIN
    -- Business events title length constraint
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'business_events_title_length'
        AND table_name = 'business_events'
    ) THEN
        ALTER TABLE public.business_events
        ADD CONSTRAINT business_events_title_length 
        CHECK (length(title) BETWEEN 3 AND 200);
    END IF;

    -- Business events description length constraint
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'business_events_description_length'
        AND table_name = 'business_events'
    ) THEN
        ALTER TABLE public.business_events
        ADD CONSTRAINT business_events_description_length 
        CHECK (length(description) <= 2000);
    END IF;
END $$;

-- Create URL validation trigger for business_events
...
-- 10. Create a secure function for public event proposals
