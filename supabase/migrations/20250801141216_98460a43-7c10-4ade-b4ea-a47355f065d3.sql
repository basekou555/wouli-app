...
-- Changer le type de price de numeric à text pour compatibilité
ALTER TABLE public.events ALTER COLUMN price TYPE TEXT;
...
-- Vérification de l'intégrité (ces requêtes ne retournent rien mais valident la structure)
DO $$
DECLARE
...
BEGIN
  ...
  RAISE NOTICE 'Migration terminée - % événements dans la table unifiée', events_count;
  
  -- Compter les profils business
  SELECT COUNT(*) INTO business_profiles_count FROM public.profiles WHERE type = 'business';
  RAISE NOTICE 'Migration terminée - % profils business configurés', business_profiles_count;
END $$;