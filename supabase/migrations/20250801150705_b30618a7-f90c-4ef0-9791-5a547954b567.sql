...
-- Supprimer la contrainte price qui cause le conflit
...
BEGIN
  ...
  RAISE NOTICE 'Migration réussie - % événements dans la table unifiée', events_count;
  
  SELECT COUNT(*) INTO business_profiles_count FROM public.profiles WHERE type = 'business';
  RAISE NOTICE 'Migration réussie - % profils business configurés', business_profiles_count;
END $$;