-- MIGRATION BACKEND FINALE ULTRA-SIMPLIFIÉE
...
-- Créer profils business depuis business_configs
...
-- Créer profils manquants pour business_events
INSERT INTO public.profiles (id, username, type, client_name, client_type, location, brand_color, features)
SELECT DISTINCT
  be.user_id,
...
SELECT 
  ...
  be.image_url, be.external_url, be.price, 
  be.views, be.likes, be.participants,
  be.user_id, 'business'::event_creator_type,
  be.created_at, be.updated_at,
  be.time, be.venue, be.custom_venue
FROM public.business_events be
WHERE NOT EXISTS (SELECT 1 FROM public.events e WHERE e.id = be.id);

-- Mettre à jour triggers
...
BEGIN
  ...
  RAISE NOTICE 'Événements: % (% business)', events_count, business_events_count;
  RAISE NOTICE 'Profils business: %', business_profiles_count;
  RAISE NOTICE '✅ App Wouli prête !';
END $$;